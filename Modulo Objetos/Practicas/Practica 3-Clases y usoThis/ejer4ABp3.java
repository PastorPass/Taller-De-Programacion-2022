...
public class ejer4Ap3 {
    
    public static void main(String[] args) {
        GeneradorAleatorio.iniciar();
        System.out.println("Ingrese la cantidad de habitaciones para el hotel");
        int N = Lector.leerInt();
        Hotel unHotel = new Hotel(N);
        Cliente unCliente;
        
        for (int i = 0; i < N; i++) {
            if (GeneradorAleatorio.generarBoolean()) {
                unCliente = new Cliente (GeneradorAleatorio.generarString(10),GeneradorAleatorio.generarInt(48000),GeneradorAleatorio.generarInt(100));
                unHotel.checkearDatosDeLaHabitacion(i, unCliente);
            }
        }
        
        System.out.println(unHotel.toString());
        unHotel.aumentarPrecio(300);
        System.out.println(unHotel.toString());
    }
}
